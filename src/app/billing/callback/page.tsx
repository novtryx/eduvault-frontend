...
// shows the result.
//
// It intentionally sits outside both (app) and (auth) route groups — it
// isn't part of the authenticated shell, but the person IS already logged
// in (this is a mid-session redirect, not a fresh visit), so it reads their
// current school from auth context rather than asking them to log in again.
...
function BillingCallbackContent() {
  ...
  const callbackQuery = useVerifyBillingCallback(
    reference,
    currentSchoolId,
  );
          ...
          {!reference ? (
            <>
              ...
              <p className="mt-1.5 text-[13.5px] text-navy-400">
                This link is missing its payment reference. If you completed a
                checkout, please check your subscription status directly.
              </p>
            </>
...
function BillingCallbackFallback() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-[400px]">
        <div className="rounded-xl border border-border bg-white p-8 text-center shadow-card">
...
export default function BillingCallbackPage() {
  return (
    <React.Suspense fallback={<BillingCallbackFallback />}>
